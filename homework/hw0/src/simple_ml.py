...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(image_filename, "rb") as img_file:
        # 用struct模块的unpack函数解析img_file的前16个字节，得到四个整数：magic_num是文件格式的标识符，img_num是图片的数量，row和col是图片的行数和列数3。
        magic_num, img_num, row, col = struct.unpack(">4i", img_file.read(16))
        assert (magic_num == 2051)
        tot_pixels = row * col
        # 2d ndarray
        # 每次从img_file中读取tot_pixels个字节，用struct.unpack函数解析成tot_pixels个无符号字节（“B”）
        # ，然后用np.array函数转换成一个numpy数组，指定数据类型为np.float32。然后用np.vstack函数将所有的数组垂直堆叠成一个二维数组X，每一行对应一张图片的像素值。
...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    # np.sum(np.log(np.sum(np.exp(Z), axis=1))) 这一部分是计算每个样本的所有类别的预测概率之和的对数，然后求和
    # np.sum(Z[np.arange(y.size), y]) 这一部分是计算每个样本的真实类别的得分，然后求和。
